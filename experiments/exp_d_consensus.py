...
def _build_judges(force_mock: bool) -> List[BaseLLMJudge]:
    """Build architecturally-diverse judges for a valid Fleiss kappa.

    The live-provider chain (HF Gemma / Cerebras / Mistral / Together /
    SambaNova / Groq / Gemini) is centralised in ``src.llm.judge_factory`` so it
    is defined exactly once across experiments B–E.

    In mock mode — or when no API key is present — three noisy mock judges stand
    in so the multi-rater agreement pipeline can still be exercised offline.
    ...
    """
    judges = build_real_judges(force_mock, tag="Exp D")

    if not judges:
        print("[Exp D] No live API keys found — using 3 mock consensus judges")
        judges = [
            ConsensusMockJudge("mock-consensus-a", accuracy=0.88, seed=101),
            ConsensusMockJudge("mock-consensus-b", accuracy=0.80, seed=202),
            ConsensusMockJudge("mock-consensus-c", accuracy=0.74, seed=303),
        ]
