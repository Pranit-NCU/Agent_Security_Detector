...
def _build_judges(force_mock: bool) -> List[BaseLLMJudge]:
    ...
    if not force_mock and gemini_key:
        try:
            ...
            print("[Exp B] Using live GeminiJudge")
        except Exception as exc:
            print(f"[Exp B] GeminiJudge unavailable ({exc}), falling back to mock")

    openrouter_key = os.getenv("OPENROUTER_API_KEY", "")
    if not force_mock and openrouter_key:
        try:
            from src.llm.openrouter_judge import OpenRouterJudge
            judges.append(OpenRouterJudge(JudgeConfig(model_name="meta-llama/llama-3-8b-instruct")))
            print("[Exp B] Using live OpenRouterJudge (llama-3-8b)")
        except Exception as exc:
            print(f"[Exp B] OpenRouterJudge unavailable ({exc}), falling back to mock")

    if not judges:
        # Offline / CI mode — three mocks at different accuracy tiers
