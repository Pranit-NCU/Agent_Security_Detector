...
def _build_judges(force_mock: bool) -> List[BaseLLMJudge]:
    ...
    if not force_mock and gemini_key:
        try:
            ...
            print(f"[Exp E] GeminiJudge unavailable ({exc})")
    if not judges:
        print("[Exp E] Using mock judges")
        judges = [
            PairedMockJudge("paired-model-alpha", accuracy=0.90, seed=444),
            PairedMockJudge("paired-model-beta",  accuracy=0.80, seed=555),
            PairedMockJudge("paired-model-gamma", accuracy=0.72, seed=666),
        ]
    return judges


# ---------------------------------------------------------------------------
# Discrimination metric helpers
# ---------------------------------------------------------------------------

def discrimination_accuracy(
    vuln_preds: list[bool],
    patch_preds: list[bool],
) -> float:
    """% of pairs where vuln is flagged AND patch is not flagged."""
    assert len(vuln_preds) == len(patch_preds)
    correct = sum(
        1 for v, p in zip(vuln_preds, patch_preds) if v and not p
    )
    return correct / len(vuln_preds) if vuln_preds else 0.0
