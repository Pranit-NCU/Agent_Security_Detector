...
from src.config import BENCHMARK_PATH, PROCESSED_DIR, ADVERSARIAL_DIR as _ADV_DIR
...
def _build_judge(force_mock: bool) -> BaseLLMJudge:
    """Exp F uses a single judge.

    Preference: Cerebras Llama 3.3 70B (code-capable, 1M tokens/day free).
    Fallback chain: SambaNova -> Groq -> Gemini -> mock.

    A code-capable Llama model is ideal here: code-specialised behaviour makes
    adversarial obfuscation results scientifically interesting.
    """
    # 1. Cerebras (Llama 3.3 70B -- primary)
    cerebras_key = os.getenv("CEREBRAS_API_KEY", "").strip()
    if not force_mock and cerebras_key:
        try:
            from src.llm.cerebras_judge import CEREBRAS_RECOMMENDED_MODELS, make_cerebras_judge
            model = CEREBRAS_RECOMMENDED_MODELS[0]
            print(f"[Exp F] Using Cerebras judge: {model}")
            return make_cerebras_judge(model)
        except Exception as exc:
            print(f"[Exp F] Cerebras judge unavailable ({exc})")

    # 2. Together AI (Llama 3.3 70B Free -- code-capable, same family as Cerebras)
    together_key = os.getenv("TOGETHER_API_KEY", "").strip()
    if not force_mock and together_key:
        try:
            from src.llm.together_judge import TOGETHER_RECOMMENDED_MODELS, make_together_judge
            model = TOGETHER_RECOMMENDED_MODELS[0]
            print(f"[Exp F] Using Together judge: {model}")
            return make_together_judge(model)
        except Exception as exc:
            print(f"[Exp F] Together judge unavailable ({exc})")

    # 3. SambaNova fallback (same Llama family, 200K tokens/day -- may 429)
    sn_key = os.getenv("SAMBANOVA_API_KEY", "").strip()
    if not force_mock and sn_key:
        try:
            from src.llm.sambanova_judge import SAMBANOVA_RECOMMENDED_MODELS, make_sambanova_judge
            model = SAMBANOVA_RECOMMENDED_MODELS[0]
            print(f"[Exp F] Using SambaNova judge (Cerebras fallback): {model}")
            return make_sambanova_judge(model)
        except Exception as exc:
            print(f"[Exp F] SambaNova judge unavailable ({exc})")

    # 3. Groq (Llama 3.1 8B -- small but fast, Llama family)
    groq_key = os.getenv("GROQ_API_KEY", "").strip()
    if not force_mock and groq_key:
        try:
            from src.llm.groq_judge import GROQ_RECOMMENDED_MODELS, make_groq_judge
            model = GROQ_RECOMMENDED_MODELS[0]
            print(f"[Exp F] Using Groq judge: {model}")
            return make_groq_judge(model)
        except Exception as exc:
            print(f"[Exp F] Groq judge unavailable ({exc})")

    # 4. Gemini (emergency fallback)
